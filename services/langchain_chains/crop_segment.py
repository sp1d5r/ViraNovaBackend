...
delete_operation_prompt = PromptTemplate(
    template="Given the transcript {transcript}, and the TikTok idea:\n{short_idea}\n\n Decide which section of the transcript needs to be deleted because it is unrelated to the TikTok idea. Avoid being over agressive with teh cropping. The transcript should still make sense: \n\n  {format_instructions}",
...
delete_operation_chain = delete_operation_prompt | model | delete_operation_parser
...
requires_cropping_chain = requires_cropping_prompt | model | requires_cropping_parser
